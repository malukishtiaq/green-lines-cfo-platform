...
export default function CostPerHireKPIPage() {
  ...
  const handleFetchKPI = async () => {
    ...
    try {
      ...
      const url = `/api/erp/kpi/cost-per-hire?connectionId=${selectedConnection}&startDate=${startDate}&endDate=${endDate}`;
        ...
        <Card
          title={
            <Space>
              ...
              <span style={{ fontSize: '20px', fontWeight: 'bold' }}>Cost per Hire KPI</span>
            </Space>
          }
          extra={<Tag color="gold" style={{ fontSize: '14px', padding: '4px 12px' }}>HR.COST_PER_HIRE</Tag>}
        >
          ...
          {kpiResult && !loading && (
            <>
              <Card type="inner" style={{ marginBottom: '24px', background: 'linear-gradient(135deg, #faad14 0%, #d48806 100%)', color: 'white' }}>
                <Row gutter={[24, 24]} justify="center">
                  <Col xs={24} sm={12} md={8}>
                    <Statistic title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>Cost per Hire</span>}
                      value={kpiResult.costPerHire} precision={2} prefix="$"
              ...
              <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
                <Col xs={24} sm={12}>
                  <Card type="inner"><Statistic title="Total Recruitment Cost" value={kpiResult.totalRecruitmentCost} precision={2} prefix="$" valueStyle={{ color: '#ff4d4f' }} /></Card>
                </Col>
                <Col xs={24} sm={12}>
                  <Card type="inner"><Statistic title="New Hires" value={kpiResult.newHires} valueStyle={{ color: '#52c41a' }} /></Card>
                </Col>
              </Row>
          ...
          {!loading && !kpiResult && !error && (
            <Card><div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ color: '#bfbfbf' }}>Select a connection and date range, then click "Calculate Cost per Hire"</p>
            </div></Card>
          )}
        </Card>
      </div>
