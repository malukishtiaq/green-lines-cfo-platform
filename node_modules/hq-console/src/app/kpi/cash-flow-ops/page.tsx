...
export default function CashFlowOpsKPIPage() {
  ...
  const handleFetchKPI = async () => {
    ...
    try {
      ...
      const url = `/api/erp/kpi/cash-flow-ops?connectionId=${selectedConnection}&startDate=${startDate}&endDate=${endDate}`;
        ...
        <Card
          title={
            <Space>
              ...
              <span style={{ fontSize: '20px', fontWeight: 'bold' }}>Cash Flow from Operations KPI</span>
            </Space>
          }
          extra={<Tag color="cyan" style={{ fontSize: '14px', padding: '4px 12px' }}>FIN.CASH_FLOW_OPS</Tag>}
        >
              ...
              <Card type="inner" style={{ marginBottom: '24px', background: 'linear-gradient(135deg, #13c2c2 0%, #08979c 100%)', color: 'white' }}>
                <Row gutter={[24, 24]} justify="center">
                  <Col xs={24} sm={12} md={8}>
                    <Statistic title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>Cash Flow from Operations</span>}
                      value={kpiResult.cashFlowOps} precision={2} prefix="$"
                      valueStyle={{ color: 'white', fontSize: '48px', fontWeight: 'bold' }}
                      prefix={kpiResult.cashFlowOps >= 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />} />
                  </Col>
                </Row>
              </Card>
              <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
                <Col xs={24} sm={12}>
                  <Card type="inner"><Statistic title="Cash Inflows" value={kpiResult.cashInflows} precision={2} prefix="$" valueStyle={{ color: '#52c41a' }} /></Card>
                </Col>
                <Col xs={24} sm={12}>
                  <Card type="inner"><Statistic title="Cash Outflows" value={kpiResult.cashOutflows} precision={2} prefix="$" valueStyle={{ color: '#ff4d4f' }} /></Card>
                </Col>
              </Row>
          ...
          {!loading && !kpiResult && !error && (
            <Card><div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ color: '#bfbfbf' }}>Select a connection and date range, then click "Calculate Cash Flow"</p>
            </div></Card>
          )}
        </Card>
      </div>
