...
export default function OpenPositionsKPIPage() {
  ...
  const handleFetchKPI = async () => {
    ...
    try {
      ...
      setError(null);
      setKpiResult(null);
      const url = `/api/erp/kpi/open-positions?connectionId=${selectedConnection}`;
        ...
        <Card
          title={
            <Space>
              ...
              <span style={{ fontSize: '20px', fontWeight: 'bold' }}>Open Positions KPI</span>
            </Space>
          }
          extra={<Tag color="purple" style={{ fontSize: '14px', padding: '4px 12px' }}>HR.OPEN_POS</Tag>}
        >
          ...
          {kpiResult && !loading && (
            <>
              <Card type="inner" style={{ marginBottom: '24px', background: 'linear-gradient(135deg, #722ed1 0%, #531dab 100%)', color: 'white' }}>
                <Row gutter={[24, 24]} justify="center">
                  <Col xs={24} sm={12} md={8}>
                    <Statistic title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>Open Positions</span>}
                      value={kpiResult.openPositions}
          ...
          {!loading && !kpiResult && !error && (
            <Card><div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ color: '#bfbfbf' }}>Select a connection, then click "Calculate Open Positions"</p>
            </div></Card>
          )}
        </Card>
      </div>
