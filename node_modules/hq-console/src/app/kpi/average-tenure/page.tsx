...
export default function AverageTenureKPIPage() {
  ...
  const handleFetchKPI = async () => {
    ...
    try {
      ...
      setError(null);
      setKpiResult(null);
      const url = `/api/erp/kpi/average-tenure?connectionId=${selectedConnection}`;
        ...
        <Card
          title={
            <Space>
              ...
              <span style={{ fontSize: '20px', fontWeight: 'bold' }}>Average Tenure KPI</span>
            </Space>
          }
          extra={<Tag color="blue" style={{ fontSize: '14px', padding: '4px 12px' }}>HR.AVG_TENURE</Tag>}
        >
          ...
          {kpiResult && !loading && (
            <>
              <Card type="inner" style={{ marginBottom: '24px', background: 'linear-gradient(135deg, #1890ff 0%, #096dd9 100%)', color: 'white' }}>
                <Row gutter={[24, 24]} justify="center">
                  <Col xs={24} sm={12} md={8}>
                    <Statistic title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>Average Tenure</span>}
                      value={kpiResult.averageTenureMonths} precision={1} suffix="months"
              ...
              <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
                <Col xs={24}>
                  <Card type="inner"><Statistic title="Total Employees" value={kpiResult.totalEmployees} valueStyle={{ color: '#1890ff' }} /></Card>
                </Col>
              </Row>
          ...
          {!loading && !kpiResult && !error && (
            <Card><div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ color: '#bfbfbf' }}>Select a connection, then click "Calculate Average Tenure"</p>
            </div></Card>
          )}
        </Card>
      </div>
