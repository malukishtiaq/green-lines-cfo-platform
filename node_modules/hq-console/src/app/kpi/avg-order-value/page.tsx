...
import { ArrowUpOutlined, ArrowDownOutlined, ReloadOutlined, DollarOutlined } from '@ant-design/icons';
...
export default function AvgOrderValueKPIPage() {
  ...
  const handleFetchKPI = async () => {
    ...
    try {
      ...
      const url = `/api/erp/kpi/avg-order-value?connectionId=${selectedConnection}&startDate=${startDate}&endDate=${endDate}`;
        ...
        <Card
          title={
            <Space>
              ...
              <span style={{ fontSize: '20px', fontWeight: 'bold' }}>Average Order Value KPI</span>
            </Space>
          }
          extra={<Tag color="green" style={{ fontSize: '14px', padding: '4px 12px' }}>SALES.AVG_ORDER_VALUE</Tag>}
        >
          ...
          {kpiResult && !loading && (
            <>
              <Card type="inner" style={{ marginBottom: '24px', background: 'linear-gradient(135deg, #52c41a 0%, #389e0d 100%)', color: 'white' }}>
                <Row gutter={[24, 24]} justify="center">
                  <Col xs={24} sm={12} md={8}>
                    <Statistic title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>Average Order Value</span>}
                      value={kpiResult.avgOrderValue} precision={2} prefix="$"
              ...
              <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
                <Col xs={24} sm={12}>
                  <Card type="inner"><Statistic title="Total Revenue" value={kpiResult.totalRevenue} precision={2} prefix="$" valueStyle={{ color: '#1890ff' }} /></Card>
                </Col>
                <Col xs={24} sm={12}>
                  <Card type="inner"><Statistic title="Total Orders" value={kpiResult.totalOrders} valueStyle={{ color: '#52c41a' }} /></Card>
                </Col>
              </Row>
          ...
          {!loading && !kpiResult && !error && (
            <Card><div style={{ textAlign: 'center', padding: '60px 20px' }}>
              <DollarOutlined style={{ fontSize: '72px', color: '#d9d9d9', marginBottom: '16px' }} />
              <h3 style={{ color: '#8c8c8c' }}>No Data Yet</h3>
              <p style={{ color: '#bfbfbf' }}>Select a connection and date range, then click "Calculate Average Order Value"</p>
            </div></Card>
          )}
        </Card>
      </div>
