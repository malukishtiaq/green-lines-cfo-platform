...
interface KPIResult {
  kpiCode: string;
  kpiName: string;
  currentPeriod: { startDate: string; endDate: string; employeeCount: number };
  previousPeriod: { startDate: string; endDate: string; employeeCount: number };
...
export default function EmployeeGrowthKPIPage() {
  ...
  const handleFetchKPI = async () => {
    ...
    try {
      ...
      const url = `/api/erp/kpi/employee-growth?connectionId=${selectedConnection}&startDate=${startDate}&endDate=${endDate}`;
        ...
        <Card
          title={
            <Space>
              ...
              <span style={{ fontSize: '20px', fontWeight: 'bold' }}>Employee Growth Rate KPI</span>
            </Space>
          }
          extra={<Tag color="green" style={{ fontSize: '14px', padding: '4px 12px' }}>HR.EMP_GROWTH%</Tag>}
        >
          ...
          {kpiResult && !loading && (
            <>
              <Card type="inner" style={{ marginBottom: '24px', background: 'linear-gradient(135deg, #52c41a 0%, #389e0d 100%)', color: 'white' }}>
                <Row gutter={[24, 24]} justify="center">
                  <Col xs={24} sm={12} md={8}>
                    <Statistic title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>Employee Growth Rate</span>}
                      value={kpiResult.employeeGrowth} precision={2} suffix="%"
                      valueStyle={{ color: 'white', fontSize: '48px', fontWeight: 'bold' }}
                      prefix={kpiResult.employeeGrowth >= 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />} />
                  </Col>
                </Row>
              </Card>
              <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
                <Col xs={24} sm={12}>
                  <Card type="inner" styles={{ header: { backgroundColor: '#e6f7ff' } }} title="Current Period">
                    <Statistic title="Employee Count" value={kpiResult.currentPeriod.employeeCount} valueStyle={{ color: '#1890ff' }} />
                ...
                <Col xs={24} sm={12}>
                  <Card type="inner" styles={{ header: { backgroundColor: '#fff7e6' } }} title="Previous Period">
                    <Statistic title="Employee Count" value={kpiResult.previousPeriod.employeeCount} valueStyle={{ color: '#fa8c16' }} />
                    <p style={{ marginTop: '8px', color: '#666', fontSize: '12px' }}>
          ...
          {!loading && !kpiResult && !error && (
            <Card><div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ color: '#bfbfbf' }}>Select a connection and date range, then click "Calculate Employee Growth"</p>
            </div></Card>
          )}
        </Card>
      </div>
