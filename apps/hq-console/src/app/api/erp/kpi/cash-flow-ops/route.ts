...
/**
 * GET /api/erp/kpi/cash-flow-ops
 * 
 * Calculates Cash Flow from Operations KPI from Odoo data
 * Formula: Cash Inflows - Cash Outflows (from operating activities)
 * 
 * Simplified: Revenue Collected - Expenses Paid
 * 
 * Query Parameters:
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cashInflows = await fetchRevenue(
      credentials.odooUrl,
    ...
    const cashOutflows = await fetchOperatingExpenses(
      credentials.odooUrl,
      credentials.odooDatabase,
      credentials.odooUsername,
      credentials.odooPassword,
    ...
    const cashFlowOps = cashInflows - cashOutflows;

    return NextResponse.json({
      success: true,
      data: {
        kpiCode: 'FIN.CASH_FLOW_OPS',
        kpiName: 'Cash Flow from Operations',
        period: {
          startDate,
          endDate,
        },
        cashInflows,
        cashOutflows,
        cashFlowOps: parseFloat(cashFlowOps.toFixed(2)),
        formula: 'Cash Inflows - Cash Outflows',
        calculation: `${cashInflows.toFixed(2)} - ${cashOutflows.toFixed(2)} = ${cashFlowOps.toFixed(2)}`,
        note: 'Simplified cash flow calculation',
      },
    });
  } catch (error: any) {
    console.error('Cash Flow KPI Error:', error);
    return NextResponse.json(
      {
        success: false,
