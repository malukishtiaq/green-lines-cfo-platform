...
/**
 * GET /api/erp/kpi/open-positions
 * 
 * Calculates Open Positions KPI from Odoo data
 * Formula: Count of open job positions
 * 
 * Query Parameters:
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: {
        kpiCode: 'HR.OPEN_POSITIONS',
        kpiName: 'Open Positions',
        asOfDate,
        openPositions,
        formula: 'Count of open job positions',
        calculation: `Total open positions: ${openPositions}`,
      },
    });
  } catch (error: any) {
    console.error('Open Positions KPI Error:', error);
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to calculate open positions' },
      { status: 500 }
    );
  }
}
...
async function fetchOpenPositions(
  ...
  const domain = [['state', '=', 'recruit']];
  const fields = ['id', 'name'];

  const records = await fetchHRJobWithFallback(
    axiosInstance,
    baseUrl,
    sessionId,
    domain,
...
async function fetchHRJobWithFallback(
  axiosInstance: any,
  baseUrl: string,
  sessionId: string,
  domain: any[],
  ...
  const endpoints = [
    {
      ...
      payload: {
        jsonrpc: '2.0',
        method: 'call',
        params: { model: 'hr.job', domain, fields, limit: 10000, sort: '', context: {} },
      },
    },
    {
      ...
      payload: {
        jsonrpc: '2.0',
        method: 'call',
        params: { model: 'hr.job', method: 'search_read', args: [], kwargs: { domain, fields } },
      },
    },
  ];
